...
function YOOMenuDropdownDecorator(&$node, $args) {
	...
	if ($node->name() == 'li') {
		...
		$sfx_params  = new YOOTemplateParameter($item_params->get('pageclass_sfx'));	
		$level       = $node->attributes('level') - $params->get('startLevel');
		$color       = $sfx_params->get('itemcolor', '');
		$columns     = (int) $sfx_params->get('columns', 1);
		...
		if (isset($node->ul[0]) && $level == 1) {
			...
			$div->_prefix = '<div class="dropdown columns'.$count.'"><div class="dropdown-t1"><div class="dropdown-t2"><div class="dropdown-t3"></div></div></div><div class="dropdown-1"><div class="dropdown-2">';
