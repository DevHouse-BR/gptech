...
class YOOMenu {
	...
	function render(&$params, $callback) {
		...
		$suffix = strtolower($params->get('class_sfx'));
		
		// use custom menu output
		if (in_array($suffix, array('accordion', 'dropdown'))) {
			
			// include decorator
			require_once(dirname(__FILE__)."/decorator/$suffix.php");				

			// generate menu output
			$xml = modMainMenuHelper::getXML($params->get('menutype'), $params, 'YOOMenu'.ucfirst($suffix).'Decorator');
			
			if ($xml) {
				$xml->addAttribute('class', 'menu menu-'.$params->get('class_sfx'));
