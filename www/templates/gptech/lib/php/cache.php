...
class YOOCache {
	...
	function process($gzip = false) {
		...
		if ($caching) {
			...
			foreach ($this->document->{$this->source} as $file => $attr) {
			...
			$this->document->{$this->source} = $source;
		}
...
class YOOCacheStylesheet extends YOOCache {
	...
	function YOOCacheStylesheet(&$document) {
		...
		// init vars
		$this->source = '_styleSheets';
		
		// only add non print stylesheet files
		$pattern = '/^.*'.str_replace('/', '\/', $this->uri).'(.*)/';
		foreach ($document->{$this->source} as $file => $attr) {
